...
const TriageQueuePage = () => {
    ...
    const PatientCard = ({ patient, category }) => (
        <div
            ...
            className={`p-6 rounded-3xl border-2 cursor-pointer transition-all hover:shadow-xl ${selectedPatient?.patient_id === patient.patient_id
            ...
            <div className="space-y-8 font-sans">
                ...
                <div className="flex justify-between items-center">
                    <div>
                        ...
                        <p className="text-slate-500 font-medium mt-1">Real-time patient priority queue with doctor booking</p>
                    </div>
                    <button
                        onClick={() => { fetchQueue(); fetchDoctors(); }}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        ...
                        {queue.critical?.length > 0 && (
                            <div>
                                <h2 className="text-2xl font-black text-red-600 mb-4 flex items-center gap-2">
                                    <AlertTriangle size={24} /> Critical Priority
                                </h2>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {queue.critical.map(patient => (
                                        <PatientCard key={patient.patient_id} patient={patient} category="critical" />
                                    ))}
                                </div>
                            </div>
                        )}
                        ...
                        {queue.medium?.length > 0 && (
                            <div>
                                <h2 className="text-2xl font-black text-orange-600 mb-4 flex items-center gap-2">
                                    <Activity size={24} /> Medium Priority
                                </h2>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {queue.medium.map(patient => (
                                        <PatientCard key={patient.patient_id} patient={patient} category="medium" />
                                    ))}
                                </div>
                            </div>
                        )}
                        ...
                        {queue.low?.length > 0 && (
                            <div>
                                <h2 className="text-2xl font-black text-emerald-600 mb-4 flex items-center gap-2">
                                    <CheckCircle size={24} /> Low Priority
                                </h2>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {queue.low.map(patient => (
                                        <PatientCard key={patient.patient_id} patient={patient} category="low" />
                                    ))}
                                </div>
                            </div>
                        )}
